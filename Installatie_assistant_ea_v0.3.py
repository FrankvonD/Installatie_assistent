...
# Functie om installaties sequentieel te starten
def start_installation():
    ...
    for idx, file_path in enumerate(selected, start=1):
        ...
        # Status updaten
        status_label.config(text=f"Bezig met: {basename}")
        # Voortgang vóór start
        ...
        # Proces starten en wachten
        proc = subprocess.Popen(cmd, shell=True)
        proc.wait()

        # Na voltooiing loggen en voortgang bijwerken
        ret = proc.returncode
    ...
    # Alles klaar
    status_label.config(text="Alle installaties voltooid")
